...
while 1:
    try:
        ...
        if int(__import__('os').stat(file).st_size) > 524288000:
            print(f'{s}this file too large!{k}')
            continue
        break
    except FileNotFoundError:
        print(f'{s}file not found!{k}')
    except PermissionError:
        print(f'{s}permission denied!{k}')
    except OSError:
        continue

while 1:
    inp = str(input(f"{s}A{k} -> Auto save all\n{s}M{k} -> Manually view and save\n{s}choose{k} > "))
    if inp.upper() == "A":
        is_auto_save = True
        break
    elif inp.upper() == "M":
...
if not data_dump:
    print(f"{s}!! this file is not Kramer/Specter !!{k}")
